...
questions_and_answers = {
    ...
    r'что делать если(.*?)нашел|как вернуть найденную вещь': 'Если вы нашли чужую вещь, разместите объявление в разделе "Находки". Укажите место и время находки, опишите предмет (но не слишком детально, чтобы настоящий владелец мог подтвердить, что вещь принадлежит ему). Дождитесь отклика настоящего владельца.',
    r'что делать если(.*?)потерял|как найти потерянную вещь': 'Если вы потеряли вещь, разместите объявление в разделе "Пропажи". Укажите место и время, когда предположительно была утеряна вещь, подробно опишите её и по возможности прикрепите фотографию. Регулярно проверяйте раздел "Находки".',
